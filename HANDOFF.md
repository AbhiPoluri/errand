...
tsc + the 27 offline suites green (revert if not) → commit on `durability-electron` → update LOOP.md →
...
- **27 offline test suites** (all green): `npm run X:test` for X in: migrate seq paths folders resume loop
  web fileops journal embed store websink ext bash clickrisk restart cap endpoint zip docwrite mcp mcpconfig
  skill models userun runroute session. (`mem`/`doc`/`ocr` need the OpenRouter key / are slow.) `npx tsc --noEmit` clean.
